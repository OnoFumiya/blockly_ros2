...
* **Mode (`mode`):** `"service"`
* **Service Name:** `spawn`
* **Interface (Message Type):** `turtlesim.Spawn` (you can use `.` or `/` as a delimiter)

Terminal execution example:

```sh
$ ros2 service call /spawn turtlesim/srv/Spawn '{x: 5.2, y: 5.8, theta: 0.0, name: "ROBOT1"}'
...
> The `turtlesim/srv/Spawn` interface includes a `theta` field, but it is omitted in the YAML above. Unconfigured fields will not appear on the block and will be sent using their **default values** (`0.0` for float types).
