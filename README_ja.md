...
* **サービス名:** `spawn`
* **Interface (メッセージ型):** `turtlesim.Spawn`（パッケージ名と型名の区切りは `/` のほか `.` でも記述可能です）
...
> [!NOTE]
> **未設定フィールドの挙動について**
> `turtlesim/srv/Spawn` には `theta` フィールドが存在しますが、上記YAMLでは設定していません。このように設定を省略したフィールドは Block 上に表示されず、送信時には**デフォルト値**（float型の場合は `0.0`）が適用されます。
...
## 参考文献
* [ROS2 Jazzy](http://wiki.ros.org/jazzy)
